...
def Conv_Block(inputs, model_width, kernel):
...
def MultiResUNet(skip_connections, model_width, model_depth, D_S, A_G, LSTM, is_transconv, kernel, alpha=1.0):
    # MultiResUNetUNet based Decoder
    levels = []
...
class AHNetWithPretrainedEncoder:
    def __init__(self, decoder_name, length, width, model_width, model_depth, problem_type='Regression', num_channels=3, output_nums=1, ds=0, ae=0, ag=0, lstm=0,
        ...
        if (self.model_depth > 5) or (self.model_depth < 1):
            raise ValueError('The depth of the model can only be discretely varied from 1 to 5')
    ...
    def ResNet50(self):
        ...
        if self.train_mode == 'pretrained_encoder':
            ...
            if self.decoder_name == 'MultiResUNet':
    ...
    def DenseNet201(self):
        ...
        if self.train_mode == 'pretrained_encoder':
            ...
            if self.decoder_name == 'MultiResUNet':
                ...
                encoder_level_1 = Conv_Block(conv, self.model_width * (2 ** 0), (3, 3))  # 224*224*32
                convs["conv1"] = encoder_level_1
            convs["conv1"] = Conv_Block(conv, self.model_width * (2 ** 0), (3, 3))  # 224*224*32
            '''Encoder Level 2'''
            conv = Base_Model.get_layer('conv1/relu').output  # 112*112*64
            if self.decoder_name == 'MultiResUNet':
                encoder_level_2 = MultiResBlock(conv, self.model_width * (2 ** 1), (3, 3), self.alpha)  # 112*112*64
    ...
    def MobileNet(self):
        ...
        if self.train_mode == 'pretrained_encoder':
            ...
            #
            encoder_level_2 = tf.keras.layers.concatenate([conv1_1, conv3], axis=-1)
            convs["conv2"] = Conv_Block(encoder_level_2, self.model_width * (2 ** 1), (3, 3))  # 112*112*64
            '''Encoder Level 3'''
            ...
            #
            encoder_level_3 = tf.keras.layers.concatenate([conv1, conv1_1], axis=-1)
            convs["conv3"] = Conv_Block(encoder_level_3, self.model_width * (2 ** 2), (3, 3))  # 56*56*128
            '''Encoder Level 4'''
            ...
            conv1_1 = tf.keras.layers.add([conv2, conv3, conv4])
            #
            encoder_level_4 = tf.keras.layers.concatenate([conv1, conv1_1], axis=-1)
            convs["conv4"] = Conv_Block(encoder_level_4, self.model_width * (2 ** 3), (3, 3))  # 28*28*256
            '''Encoder Level5'''
            ...
            #
            encoder_level_5 = tf.keras.layers.concatenate([conv1, conv1_1], axis=-1)
            convs["conv5"] = Conv_Block(encoder_level_5, self.model_width * (2 ** 4), (3, 3))  # 14*14*512
        ...
        '''Latent Layers'''
        conv = dense_block(conv, self.model_width * (2 ** self.model_depth), (3, 3), self.dense_loop - 1)
        # conv = Conv_Block(conv, self.model_width * (2 ** self.model_depth), (3, 3))
        # conv = Conv_Block(conv, self.model_width * (2 ** self.model_depth), (3, 3))
    ...
    def MobileNetV2(self):
        ...
        if self.train_mode == 'pretrained_encoder':
            ...
            #
            encoder_level_5 = tf.keras.layers.concatenate([conv1, conv1_1, conv1_2], axis=-1)
            convs["conv5"] = Conv_Block(encoder_level_5, self.model_width * (2 ** 4), (3, 3))  # 14*14*512
    ...
    def MobileNetV3Small(self):
        ...
        if self.train_mode == 'pretrained_encoder':
            ...
            #
            encoder_level_3 = tf.keras.layers.concatenate([conv1, conv2], axis=-1)
            convs["conv3"] = Conv_Block(encoder_level_3, self.model_width * (2 ** 2), (3, 3))  # 56*56*128
            '''Encoder Level 4'''
            ...
            #
            encoder_level_4 = tf.keras.layers.concatenate([conv1, conv1_1, conv4], axis=-1)
            convs["conv4"] = Conv_Block(encoder_level_4, self.model_width * (2 ** 3), (3, 3))  # 28*28*256
            '''Encoder Level5'''
            ...
            #
            encoder_level_5 = tf.keras.layers.concatenate([conv1, conv1_1, conv1_2, conv1_3, conv10], axis=-1)
            convs["conv5"] = Conv_Block(encoder_level_5, self.model_width * (2 ** 4), (3, 3))  # 14*14*512
    ...
    def MobileNetV3Large(self):
        ...
        if self.train_mode == 'pretrained_encoder':
            ...
            #
            encoder_level_4 = tf.keras.layers.concatenate([conv1, conv1_1, conv6], axis=-1)
            convs["conv4"] = Conv_Block(encoder_level_4, self.model_width * (2 ** 3), (3, 3))  # 28*28*256
            '''Encoder Level5'''
            ...
            encoder_level_5 = tf.keras.layers.concatenate([conv1, conv1_1, conv1_2, conv1_3, conv1_4], axis=-1)
            convs["conv5"] = Conv_Block(encoder_level_5, self.model_width * (2 ** 4), (3, 3))  # 14*14*512
    ...
    def InceptionV3(self):
        ...
        if self.train_mode == 'pretrained_encoder':
            ...
            #
            encoder_level_2 = tf.keras.layers.concatenate([conv3, conv1_1], axis=-1)
            convs["conv2"] = Conv_Block(encoder_level_2, self.model_width * (2 ** 1), (3, 3))  # 112*112*64
            '''Encoder Level 3'''
            ...
            #
            encoder_level_4 = tf.keras.layers.concatenate([conv1, conv1_1, conv1_2, conv1_3], axis=-1)
            convs["conv4"] = Conv_Block(encoder_level_4, self.model_width * (2 ** 3), (3, 3))  # 28*28*256
            '''Encoder Level5'''
            ...
            conv1_3 = tf.keras.layers.add([conv35, conv36, conv37, conv38, conv39, conv40, conv41, conv42, conv43, conv44,
                                           conv45, conv46])
            #
            encoder_level_5 = tf.keras.layers.concatenate([conv1, conv2, conv1_1, conv1_2, conv1_3], axis=-1)
            convs["conv5"] = Conv_Block(encoder_level_5, self.model_width * (2 ** 4), (3, 3))  # 14*14*512
    ...
    def InceptionResNetV2(self):
        ...
        if self.train_mode == 'pretrained_encoder':
            ...
            #
            encoder_level_4 = tf.keras.layers.concatenate([conv1_1, conv1_2, conv1_3, conv1_4, conv1_5], axis=-1)
            convs["conv4"] = Conv_Block(encoder_level_4, self.model_width * (2 ** 3), (3, 3))  # 28*28*256
            '''Encoder Level5'''
            ...
            #
            encoder_level_5 = tf.keras.layers.concatenate([conv1_1, conv1_2, conv1_3, conv1_4, conv1_5, conv86], axis=-1)
            convs["conv5"] = Conv_Block(encoder_level_5, self.model_width * (2 ** 4), (3, 3))  # 14*14*512
    ...
    def EfficientNetB0(self):
        ...
        if self.train_mode == 'pretrained_encoder':
            ...
            encoder_level_2 = tf.keras.layers.concatenate([conv1, conv2], axis=-1)
            convs["conv2"] = Conv_Block(encoder_level_2, self.model_width * (2 ** 1), (3, 3))  # 112*112*64
            '''Encoder Level 3'''
    ...
    def EfficientNetB1(self):
        ...
        if self.train_mode == 'pretrained_encoder':
            ...
            encoder_level_2 = tf.keras.layers.concatenate([conv1, conv1_1, conv4], axis=-1)
            convs["conv2"] = Conv_Block(encoder_level_2, self.model_width * (2 ** 1), (3, 3))  # 112*112*64
            '''Encoder Level 3'''
            conv1 = Base_Model.get_layer('block2a_activation').output  # 56*56*96
            #
    ...
    def EfficientNetB4(self):
        ...
        '''Input'''
        inputs = tf.keras.Input((self.length, self.width, 3))
    ...
    def EfficientNetB5(self):
        ...
        if self.train_mode == 'pretrained_encoder':
            ...
            encoder_level_2 = tf.keras.layers.concatenate([conv1_1, conv1_2, conv5], axis=-1)
            convs["conv2"] = Conv_Block(encoder_level_2, self.model_width * (2 ** 1), (3, 3))  # 112*112*64
            '''Encoder Level 3'''
            conv1 = Base_Model.get_layer('block2a_activation').output  # 56*56*144
            #
    ...
    def EfficientNetB7(self):
        ...
        if self.train_mode == 'pretrained_encoder':
            ...
            encoder_level_2 = tf.keras.layers.concatenate([conv1_1, conv1_2, conv6], axis=-1)
            convs["conv2"] = Conv_Block(encoder_level_2, self.model_width * (2 ** 1), (3, 3))  # 112*112*64
            '''Encoder Level 3'''
    ...
    def EfficientNetV2B0(self):
        ...
        if self.train_mode == 'pretrained_encoder':
            ...
            #
            encoder_level_5 = tf.keras.layers.concatenate([conv1, conv1_1, conv1_2, conv1_3], axis=-1)
            convs["conv5"] = Conv_Block(encoder_level_5, self.model_width * (2 ** 4), (3, 3))  # 14*14*512
    ...
    def EfficientNetV2B1(self):
        ...
        if self.train_mode == 'pretrained_encoder':
            ...
            conv1_1 = tf.keras.layers.add([conv2, conv3])
            #
            encoder_level_2 = tf.keras.layers.concatenate([conv1, conv1_1], axis=-1)
            convs["conv2"] = Conv_Block(encoder_level_2, self.model_width * (2 ** 1), (3, 3))  # 112*112*64
            '''Encoder Level 3'''
            conv1 = Base_Model.get_layer('block2a_expand_activation').output  # 56*56*64
            #
    ...
    def EfficientNetV2L(self):
        ...
        '''Latent Layers'''
        conv = dense_block(conv, self.model_width * (2 ** self.model_depth), (3, 3), self.dense_loop - 1)  # 7*7*1024
        # conv = Conv_Block(conv, self.model_width * (2 ** self.model_depth), (3, 3))
        # conv = Conv_Block(conv, self.model_width * (2 ** self.model_depth), (3, 3))
    ...
    def CheXNet(self):
        ...
        if self.train_mode == 'pretrained_encoder':
            ...
            convs["conv1"] = Conv_Block(conv, self.model_width * (2 ** 0), (3, 3))  # 224*224*32
            '''Encoder Level 2'''
            encoder_level_2 = Base_Model.get_layer('conv1/relu').output  # 112*112*64
            convs["conv2"] = Conv_Block(encoder_level_2, self.model_width * (2 ** 1), (3, 3))  # 112*112*64
            '''Encoder Level 3'''
            conv1 = Base_Model.get_layer('conv2_block1_1_relu').output  # 56*56*128
            conv2 = Base_Model.get_layer('conv2_block2_1_relu').output  # 56*56*128
            ...
            conv10 = Base_Model.get_layer('pool2_relu').output  # 56*56*256
            #
            encoder_level_3 = tf.keras.layers.concatenate([conv1_1, conv10], axis=-1)
            convs["conv3"] = Conv_Block(encoder_level_3, self.model_width * (2 ** 2), (3, 3))
            '''Encoder Level 4'''
            conv1 = Base_Model.get_layer('conv3_block1_0_relu').output  # 28*28*128
            conv2 = Base_Model.get_layer('conv3_block1_1_relu').output  # 28*28*128
            conv3 = Base_Model.get_layer('conv3_block2_1_relu').output  # 28*28*128
            ...
            conv14 = Base_Model.get_layer('pool3_relu').output  # 28*28*512
            #
            encoder_level_4 = tf.keras.layers.concatenate([conv1_1, conv14], axis=-1)
            convs["conv4"] = Conv_Block(encoder_level_4, self.model_width * (2 ** 3), (3, 3))
            '''Encoder Level 5'''
            conv1 = Base_Model.get_layer('conv4_block1_1_relu').output  # 14*14*128
            conv2 = Base_Model.get_layer('conv4_block2_1_relu').output  # 14*14*128
            conv3 = Base_Model.get_layer('conv4_block3_1_relu').output  # 14*14*128
            ...
            #
            conv25 = Base_Model.get_layer('pool4_relu').output  # 14*14*512
            encoder_level_5 = tf.keras.layers.concatenate([conv1_1, conv25], axis=-1)
            convs["conv5"] = Conv_Block(encoder_level_5, self.model_width * (2 ** 4), (3, 3))
...
if __name__ == '__main__':
    ...
    decoder_name = 'UNetPP'  # Decoder Architecture (UNet, UNetPP, etc.)
    model_width = 32  # Width of the Initial Layer, subsequent layers start from here
    model_depth = 3  # Depth or Number of Layers in the Model (Maximum 5, Minimum 1)
    ...
    Model = UNetWithPretrainedEncoder(decoder_name, length, width, model_width, model_depth, problem_type=problem_type, output_nums=output_nums,
                                      ...
                                      is_base_model_trainable=base_model_trainable).DenseNet201()
