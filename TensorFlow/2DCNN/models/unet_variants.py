...
def encoder_block_pretrained_level_1(inputs, conv_inputs, decoder_name, model_width, model_depth, alpha, q):
    conv = inputs
    convs = conv_inputs
...
def encoder_block_pretrained_level_2(inputs, conv_inputs, decoder_name, model_width, model_depth, alpha, q):
    conv = inputs
    convs = conv_inputs
    if (decoder_name == 'MultiResUNet') or (decoder_name == 'MultiResUNet3P'):
...
def encoder_block_pretrained_level_3(inputs, conv_inputs, decoder_name, model_width, model_depth, alpha, q):
    conv = inputs
    convs = conv_inputs
    if (decoder_name == 'MultiResUNet') or (decoder_name == 'MultiResUNet3P'):
...
def encoder_block_pretrained_level_4(inputs, conv_inputs, decoder_name, model_width, model_depth, alpha, q):
    conv = inputs
    convs = conv_inputs
    if (decoder_name == 'MultiResUNet') or (decoder_name == 'MultiResUNet3P'):
...
def encoder_block_pretrained_level_5(inputs, conv_inputs, decoder_name, model_width, model_depth, alpha, q):
    conv = inputs
    convs = conv_inputs
    if (decoder_name == 'MultiResUNet') or (decoder_name == 'MultiResUNet3P'):
...
class unet_model_builder:
    ...
    def ResNet50(self):
        ...
        if self.train_mode == 'pretrained_encoder':
            ...
            convs["conv1"] = encoder_block_pretrained_level_1(conv, convs, self.decoder_name, self.model_width, self.model_depth, self.alpha, self.q)
            # Encoder Level 2
            conv = Base_Model.get_layer('conv1_relu').output  # (w/2)*(h/2)*(k*2)
            convs["conv2"] = encoder_block_pretrained_level_2(conv, convs, self.decoder_name, self.model_width, self.model_depth, self.alpha, self.q)
            # Encoder Level 3
            conv = Base_Model.get_layer('conv2_block3_out').output  # (w/4)*(h/4)*(k*4)
            convs["conv3"] = encoder_block_pretrained_level_3(conv, convs, self.decoder_name, self.model_width, self.model_depth, self.alpha, self.q)
            # Encoder Level 4
            conv = Base_Model.get_layer('conv3_block4_out').output  # (w/8)*(h/8)*(k*8)
            convs["conv4"] = encoder_block_pretrained_level_4(conv, convs, self.decoder_name, self.model_width, self.model_depth, self.alpha, self.q)
            # Encoder Level 5
            conv = Base_Model.get_layer('conv4_block6_out').output  # (w/16)*(h/16)*(k*16)
            convs["conv5"] = encoder_block_pretrained_level_5(conv, convs, self.decoder_name, self.model_width, self.model_depth, self.alpha, self.q)
            # Bottleneck
            bottom = []
            if self.model_depth == 1:
                bottom = convs["conv2"]
