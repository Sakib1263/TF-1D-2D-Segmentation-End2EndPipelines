...
def MultiResBlock(inputs, model_width, kernel, alpha):
    # MultiRes Block
    # U {int} -- Number of filters in a corrsponding FPN stage
...
def FPN(skip_connections, model_width, model_depth, D_S, A_G, LSTM, is_transconv):
    # FPN based Decoder
    deconvs = {}
    levels = []
    ...
    for j in range(0, model_depth):
        ...
        if LSTM == 1:
            ...
            deconv = Add_Block(deconv, skip_connection)
    ...
    for k in range(2, (model_depth + 1)):
        deconv_temp = deconvs["deconv%s" % k]
        deconv_tot = upConv_Block(deconv_tot)
        deconv_tot = Concat_Block(deconv_tot, deconv_temp)
    return deconv_tot, levels


def SelfFPN(skip_connections, model_width, model_depth, D_S, is_transconv, q):
    # FPN based Decoder
    deconvs = {}
    levels = []
    convs_list = list(skip_connections.values())
...
def encoder_block_pretrained(inputs, decoder_name, model_width, alpha, depth, q):
    conv = inputs
    depth = np.int_(depth)
    if (decoder_name[0:4] == 'Self'):
        conv = Oper2D(model_width * (2 ** (depth - 1)), (1, 1), q=q)(conv)
    else:
        conv = Conv_Block(conv, model_width * (2 ** (depth - 1)), (1, 1), bn=False)
    return conv


def decoder_block(convs, decoder_name, model_width, model_depth, D_S, A_G, LSTM, is_transconv, alpha, q):
    if decoder_name == 'FPN':
        deconv, levels = FPN(convs, model_width, model_depth, D_S, A_G, LSTM, is_transconv)
    elif decoder_name == 'SelfFPN':
        deconv, levels = SelfFPN(convs, model_width, model_depth, D_S, is_transconv, q)
    return deconv, levels


class fpn_model_builder:
    def __init__(self, 
                 decoder_name, 
                 length, 
                 width, 
        ...
        # decoder_name: Name of the decoder model e.g., FPN, Self-FPN, etc.
        ...
        # alpha: This Parameter is only for MultiResFPN, default value is 1
    ...
    def ResNet50(self):
        # FPN Variants with ResNet50 ImageNet Trained Encoder from TF Repository
        if self.length == 0:
        ...
        if self.train_mode == 'pretrained_encoder':
            ...
            convs["conv1"] = encoder_block_pretrained(conv, self.decoder_name, self.model_width, self.model_depth, self.alpha, 1, self.q)
            # Encoder Level 2
            conv = Base_Model.get_layer('conv1_relu').output  # (w/2)*(h/2)*(k*2)
            convs["conv2"] = encoder_block_pretrained(conv, self.decoder_name, self.model_width, self.model_depth, self.alpha, 2, self.q)
            # Encoder Level 3
            conv = Base_Model.get_layer('conv2_block3_out').output  # (w/4)*(h/4)*(k*4)
            convs["conv3"] = encoder_block_pretrained(conv, self.decoder_name, self.model_width, self.model_depth, self.alpha, 3, self.q)
            # Encoder Level 4
            conv = Base_Model.get_layer('conv3_block4_out').output  # (w/8)*(h/8)*(k*8)
            convs["conv4"] = encoder_block_pretrained(conv, self.decoder_name, self.model_width, self.model_depth, self.alpha, 4, self.q)
            # Encoder Level 5
            conv = Base_Model.get_layer('conv4_block6_out').output  # (w/16)*(h/16)*(k*16)
            convs["conv5"] = encoder_block_pretrained(conv, self.decoder_name, self.model_width, self.model_depth, self.alpha, 5, self.q)
            # Bottleneck
            bottom = []
            if self.model_depth == 1:
                bottom = convs["conv2"]
    ...
    def ResNet50V2(self):
        # FPN Variants with ResNet50V2 ImageNet Trained Encoder from TF Repository
        if self.length == 0:
            raise ValueError("Please Check the Values of the Input Parameters!")
        convs = {}
    ...
    def ResNet101(self):
        # FPN Variants with ResNet101 ImageNet Trained Encoder from TF Repository
        if self.length == 0:
            raise ValueError("Please Check the Values of the Input Parameters!")
    ...
    def ResNet101V2(self):
        # FPN Variants with ResNet101V2 ImageNet Trained Encoder from TF Repository
        if self.length == 0:
            raise ValueError("Please Check the Values of the Input Parameters!")
        convs = {}
    ...
    def ResNet152(self):
        # FPN Variants with ResNet152 ImageNet Trained Encoder from TF Repository
        if self.length == 0:
            raise ValueError("Please Check the Values of the Input Parameters!")
        convs = {}
    ...
    def ResNet152V2(self):
        # FPN Variants with ResNet152V2 ImageNet Trained Encoder from TF Repository
        if self.length == 0:
            raise ValueError("Please Check the Values of the Input Parameters!")
        convs = {}
    ...
    def VGG16(self):
        # FPN Variants with VGG16 ImageNet Trained Encoder from TF Repository
        if self.length == 0:
            raise ValueError("Please Check the Values of the Input Parameters!")
        convs = {}
    ...
    def VGG19(self):
        # FPN Variants with VGG19 ImageNet Trained Encoder from TF Repository
        if self.length == 0:
            raise ValueError("Please Check the Values of the Input Parameters!")
        convs = {}
    ...
    def DenseNet121(self):
        # FPN Variants with DenseNet121 ImageNet Trained Encoder from TF Repository
        if self.length == 0:
            raise ValueError("Please Check the Values of the Input Parameters!")
        convs = {}
    ...
    def DenseNet169(self):
        # FPN Variants with DenseNet169 ImageNet Trained Encoder from TF Repository
        if self.length == 0:
            raise ValueError("Please Check the Values of the Input Parameters!")
        convs = {}
    ...
    def DenseNet201(self):
        # FPN Variants with DenseNet201 ImageNet Trained Encoder from TF Repository
        if self.length == 0:
            raise ValueError("Please Check the Values of the Input Parameters!")
        convs = {}
    ...
    def MobileNet(self):
        # FPN Variants with MobileNet ImageNet Trained Encoder from TF Repository
        if self.length == 0:
            raise ValueError("Please Check the Values of the Input Parameters!")
        convs = {}
    ...
    def MobileNetV2(self):
        # FPN Variants with MobileNetV2 ImageNet Trained Encoder from TF Repository
        if self.length == 0:
            raise ValueError("Please Check the Values of the Input Parameters!")
        convs = {}
    ...
    def MobileNetV3Small(self):
        # FPN Variants with MobileNetV3_Small ImageNet Trained Encoder from TF Repository
        if self.length == 0:
            raise ValueError("Please Check the Values of the Input Parameters!")
        convs = {}
    ...
    def MobileNetV3Large(self):
        # FPN Variants with MobileNetV3_Large ImageNet Trained Encoder from TF Repository
        if self.length == 0:
            raise ValueError("Please Check the Values of the Input Parameters!")
        convs = {}
    ...
    def InceptionV3(self):
        # FPN Variants with Inception V3 ImageNet Trained Encoder from TF Repository
        if self.length == 0:
            raise ValueError("Please Check the Values of the Input Parameters!")
        convs = {}
    ...
    def InceptionResNetV2(self):
        # FPN Variants with InceptionResNetV2 ImageNet Trained Encoder from TF Repository
        if self.length == 0:
            raise ValueError("Please Check the Values of the Input Parameters!")
        convs = {}
    ...
    def EfficientNetB0(self):
        # FPN Variants with EfficientNetB0 ImageNet Trained Encoder from TF Repository
        if self.length == 0:
            raise ValueError("Please Check the Values of the Input Parameters!")
        convs = {}
    ...
    def EfficientNetB1(self):
        # FPN Variants with EfficientNetB1 ImageNet Trained Encoder from TF Repository
        if self.length == 0:
            raise ValueError("Please Check the Values of the Input Parameters!")
        convs = {}
    ...
    def EfficientNetB2(self):
        # FPN Variants with EfficientNetB2 ImageNet Trained Encoder from TF Repository
        if self.length == 0:
            raise ValueError("Please Check the Values of the Input Parameters!")
        convs = {}
    ...
    def EfficientNetB3(self):
        # FPN Variants with EfficientNetB3 ImageNet Trained Encoder from TF Repository
        if self.length == 0:
            raise ValueError("Please Check the Values of the Input Parameters!")
        convs = {}
    ...
    def EfficientNetB4(self):
        # FPN Variants with EfficientNetB4 ImageNet Trained Encoder from TF Repository
        if self.length == 0:
            raise ValueError("Please Check the Values of the Input Parameters!")
        convs = {}
    ...
    def EfficientNetB5(self):
        # FPN Variants with EfficientNetB5 ImageNet Trained Encoder from TF Repository
        if self.length == 0:
            raise ValueError("Please Check the Values of the Input Parameters!")
        convs = {}
    ...
    def EfficientNetB6(self):
        # FPN Variants with EfficientNetB6 ImageNet Trained Encoder from TF Repository
        if self.length == 0:
            raise ValueError("Please Check the Values of the Input Parameters!")
        convs = {}
    ...
    def EfficientNetB7(self):
        # FPN Variants with EfficientNetB7 ImageNet Trained Encoder from TF Repository
        if self.length == 0:
            raise ValueError("Please Check the Values of the Input Parameters!")
        convs = {}
    ...
    def EfficientNetV2B0(self):
        # FPN Variants with EfficientNetV2B0 ImageNet Trained Encoder from TF Repository
        if self.length == 0:
            raise ValueError("Please Check the Values of the Input Parameters!")
        convs = {}
    ...
    def EfficientNetV2B1(self):
        # FPN Variants with EfficientNetV2B1 ImageNet Trained Encoder from TF Repository
        if self.length == 0:
            raise ValueError("Please Check the Values of the Input Parameters!")
        convs = {}
    ...
    def EfficientNetV2B2(self):
        # FPN Variants with EfficientNetV2B2 ImageNet Trained Encoder from TF Repository
        if self.length == 0:
            raise ValueError("Please Check the Values of the Input Parameters!")
        convs = {}
    ...
    def EfficientNetV2B3(self):
        # FPN Variants with EfficientNetV2B3 ImageNet Trained Encoder from TF Repository
        if self.length == 0:
            raise ValueError("Please Check the Values of the Input Parameters!")
        convs = {}
    ...
    def EfficientNetV2S(self):
        # FPN Variants with EfficientNetV2S (Small) ImageNet Trained Encoder from TF Repository
        if self.length == 0:
            raise ValueError("Please Check the Values of the Input Parameters!")
        convs = {}
    ...
    def EfficientNetV2M(self):
        # FPN Variants with EfficientNetV2M (Medium) ImageNet Trained Encoder from TF Repository
        if self.length == 0:
            raise ValueError("Please Check the Values of the Input Parameters!")
        convs = {}
    ...
    def EfficientNetV2L(self):
        # FPN Variants with EfficientNetV2L (Large) ImageNet Trained Encoder from TF Repository
        if self.length == 0:
            raise ValueError("Please Check the Values of the Input Parameters!")
        convs = {}
    ...
    def CheXNet(self):
        # FPN Variants with DenseNet121 based CheXNet Trained Encoder from their GitHub Repository
        if self.length == 0:
            raise ValueError("Please Check the Values of the Input Parameters!")
        convs = {}
...
if __name__ == '__main__':
    ...
    decoder_name = 'SelfFPN'  # Decoder Architecture (FPN, SelfFPN, etc.)
    ...
    Model = fpn_model_builder(decoder_name, 
                               length, 
                               width, 
                               model_width, 
                               model_depth, 
