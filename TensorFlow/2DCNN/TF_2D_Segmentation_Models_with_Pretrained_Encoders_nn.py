...
def conv2d(inputs, in_channels, out_channels, kernel_size=3, strides=1, bn=True, activation_fun='relu'):
    # 2D Convolutional Block
    x = tf.nn.conv2d(inputs, filters=[kernel_size, kernel_size, in_channels, out_channels], strides=[1, strides, strides, 1], padding='same')
    if bn == True:
        x = batchnorm(x)
...
def trans_conv2d(inputs, in_channels, out_channels, kernel_size=2, strides=2, bn=True, activation_fun='relu'):
    # 2D Transposed Convolutional Block, used instead of UpSampling
    # x = tf.keras.layers.Conv2DTranspose(model_width, (2, 2), strides=(2, 2), padding='same')
    x = tf.nn.conv2d_transpose(inputs, filters=[kernel_size, kernel_size, out_channels, in_channels], strides=[1, strides, strides, 1], padding='same')
    if bn == True:
        x = batchnorm(x)
    if (activation_fun == 'relu'):
        # x = tf.nn.relu(x)
...
def upconv2d(inputs, size=(2, 2), interpolation_mode='bilinear'):
    # 2D UpSampling Block
...
def batchnorm(input_tensor):
    ...
    mean_tensor = tf.math.reduce_mean(input_tensor)
    variance_tensor = tf.math.reduce_variance(input_tensor)
    output = tf.nn.batch_normalization(input_tensor, mean_tensor, variance_tensor)
...
def concat_block(input1, *argv):
    ...
    for arg in range(0, len(argv)):
        cat = tf.concat([cat, argv[arg]], axis=-1)

    return cat


def extract_features(inputs, model_width, feature_number):
    # Feature Extraction Block for the AutoEncoder Mode748777777777777777
    shape = inputs.shape
    ...
    latent = tf.Reshape((shape[1], shape[2], model_width))(latent)

    return latent


def dense_block(inputs, in_channels, out_channels, kernel_size, num_layers):
    output = conv2d(inputs, in_channels, out_channels, kernel_size)
    for _ in range(num_layers):
        cb = conv2d(output, in_channels, out_channels, kernel_size)
        output = tf.keras.layers.add([output, cb])

    return output


def attention_block(skip_connection, gating_signal, kernel_size, num_filters, multiplier):
    # Guided Attention Block
    conv1x1_1 = tf.nn.conv2d(skip_connection, filters=[kernel_size, kernel_size, num_filters * multiplier, num_filters * multiplier], strides=[1, 2, 2, 1], padding='valid')
    conv1x1_1 = batchnorm(conv1x1_1)
    conv1x1_2 = tf.nn.conv2d(gating_signal, filters=[kernel_size, kernel_size, num_filters * multiplier, num_filters * multiplier], strides=[1, 1, 1, 1], padding='valid')
    conv1x1_2 = batchnorm(conv1x1_2)
    conv1_2 = tf.math.add([conv1x1_1, conv1x1_2])
    conv1_2 = tf.nn.relu6(conv1_2)
    conv1_2 = tf.nn.conv2d(conv1_2, filters=[kernel_size, kernel_size, num_filters * multiplier, num_filters * multiplier], strides=[1, 1, 1, 1], padding='valid')
    conv1_2 = batchnorm(conv1_2)
    conv1_2 = tf.math.sigmoid(conv1_2)
    resampler1 = upconv2d(conv1_2)
    resampler2 = trans_conv2d(conv1_2, 1)
    resampler = tf.math.add([resampler1, resampler2])
    out = tf.math.multiply(skip_connection,resampler)
...
def multiresblock(inputs, model_width, kernel, alpha):
    ...
    shortcut = inputs
    shortcut = conv2d(shortcut, int(w * 0.167) + int(w * 0.333) + int(w * 0.5), (1, 1))

    conv3x3 = conv2d(inputs, int(w * 0.167), kernel)
    conv5x5 = conv2d(conv3x3, int(w * 0.333), kernel)
    conv7x7 = conv2d(conv5x5, int(w * 0.5), kernel)
...
def respath(inputs, model_depth, model_width, kernel):
    ...
    shortcut = inputs
    shortcut = conv2d(shortcut, model_width, (1, 1))

    out = conv2d(inputs, model_width, kernel)
    out = tf.keras.layers.Add()([shortcut, out])
    out = tf.keras.layers.Activation('relu')(out)
    out = tf.keras.layers.BatchNormalization()(out)
    ...
    for _ in range(1, model_depth):
        ...
        out = conv2d(out, model_width, kernel)
        out = tf.keras.layers.Add()([shortcut, out])
        out = tf.keras.layers.Activation('relu')(out)
        out = tf.keras.layers.BatchNormalization()(out)
...
def UNet(skip_connections, model_width, model_depth, D_S, A_G, LSTM, is_transconv):
    ...
    for j in range(0, model_depth):
        ...
        deconv = conv2d(deconv, model_width * (2 ** (model_depth - j - 1)), (3, 3))
        # deconv = conv2d(deconv, model_width * (2 ** (model_depth - j - 1)), (3, 3))
        ...
        for j in range(0, (model_depth - i + 1)):
            if i == 1:
                ...
                if is_transconv:
                    ...
                    deconv = upconv2d(convs_list[j + 1])
                SC_Shape = skip_connection.shape
                DCNV_Shape = deconv.shape
                if LSTM == 1:
                    ...
                    deconv = concat_block(deconv, skip_connection)
                deconv = conv2d(deconv, model_width * (2 ** j), (3, 3))
                # deconv = conv2d(deconv, model_width * (2 ** j), (3, 3))
                ...
                if is_transconv:
                    ...
                    deconv = upconv2d(deconvs["deconv%s%s" % ((j + 1), (i - 1))])
                SC_Shape = skip_connection.shape
                DCNV_Shape = deconv.shape
                if LSTM == 1:
                    x1 = tf.keras.layers.Reshape(target_shape=(1, SC_Shape[1], SC_Shape[2], model_width * (2 ** j)))(skip_connection)
...
def UNet3P(skip_connections, model_width, model_depth, D_S):
    ...
    for j in range(0, model_depth):
        ...
        skip_connections_all = conv2d(skip_connections_all, model_width * (2 ** 0), (3, 3))
        for k in range(0, model_depth - j - 1):
            ...
            skip_connection = conv2d(skip_connection, model_width * (2 ** 0), (3, 3))
            skip_connections_all = tf.keras.layers.concatenate([skip_connections_all, skip_connection], axis=-1)
        deconv_tot = conv2d(deconv, model_width * (2 ** 0), (3, 3))
        deconv_tot = upconv2d(deconv_tot, size=(2 ** 1, 2 ** 1), interpolation_mode='bilinear')
        ...
        if j > 0:
            for m in range(0, j):
                deconv = conv2d(deconvs["deconv%s" % m], model_width * (2 ** 0), (3, 3))
                deconv = upconv2d(deconv, size=((2 ** (j - m)), (2 ** (j - m))), interpolation_mode='bilinear')
                deconv = tf.keras.layers.Activation('sigmoid')(deconv)
                deconv_tot = tf.keras.layers.concatenate([deconv_tot, deconv], axis=-1)
        deconv = conv2d(deconv_tot, model_width * (model_depth + 1), (3, 3))
            ...
            if i == 1:
                ...
                if (i + j) == (model_depth) and (j != (model_depth - 1)):
                    for m in range(1, i - 1):
                        temp_deconv = upconv2d(deconvs_skip["deconv_skip%s" % m], size=(2 ** (i - m)))
...
class UNetWithPretrainedEncoder:
    ...
    def ResNet50(self):
        ...
        if self.train_mode == 'pretrained_encoder':
            ...
            # conv = conv2d(conv, self.model_width * (2 ** 0), (3, 3))
            if (self.decoder_name == 'MultiResUNet') or (self.decoder_name == 'MultiResUNet3P'):
                ...
                encoder_level_1 = conv2d(conv, self.model_width * (2 ** 0), (3, 3))  # iw*ih*mw
            ...
            if (self.decoder_name == 'MultiResUNet') or (self.decoder_name == 'MultiResUNet3P'):
                ...
                conv = conv2d(conv, self.model_width * (2 ** 1), (3, 3))  # (w/2)*(h/2)*(mw*2)
                for k in range(1, 2):
                ...
                encoder_level_2 = conv2d(conv, self.model_width * (2 ** 1), (3, 3))  # (w/2)*(h/2)*(mw*2)
            ...
            if (self.decoder_name == 'MultiResUNet') or (self.decoder_name == 'MultiResUNet3P'):
                ...
                conv = conv2d(conv, self.model_width * (2 ** 2), (3, 3))  # (w/4)*(h/4)*(mw*4)
                for k in range(1, 3):
                    conv_temp = convs["conv%s" %k]
                    if self.decoder_name == 'AHNet':
                        conv_temp = respath(conv_temp, self.model_depth-k, self.model_width*(2 ** 0), (3, 3))
                    ...
                    conv_temp = tf.keras.layers.Activation('sigmoid')(conv_temp)
                    conv = tf.keras.layers.concatenate([conv, conv_temp], axis=-1)
                encoder_level_3 = conv2d(conv, self.model_width * (2 ** 2), (3, 3))  # (w/4)*(h/4)*(mw*4)
            ...
            if (self.decoder_name == 'MultiResUNet') or (self.decoder_name == 'MultiResUNet3P'):
                ...
                conv = conv2d(conv, self.model_width * (2 ** 3), (3, 3))  # (w/8)*(h/8)*(mw*8)
                for k in range(1, 4):
                    conv_temp = convs["conv%s" %k]
                    if self.decoder_name == 'AHNet':
                        conv_temp = respath(conv_temp, self.model_depth-k, self.model_width*(2 ** 0), (3, 3))
                    ...
                    conv_temp = tf.keras.layers.Activation('sigmoid')(conv_temp)
                    conv = tf.keras.layers.concatenate([conv, conv_temp], axis=-1)
                encoder_level_4 = conv2d(conv, self.model_width * (2 ** 3), (3, 3))  # (w/8)*(h/8)*(mw*8)
            ...
            if (self.decoder_name == 'MultiResUNet') or (self.decoder_name == 'MultiResUNet3P'):
                ...
                conv = conv2d(conv, self.model_width * (2 ** 4), (3, 3))  # (w/16)*(h/16)*(mw*16)
                for k in range(1, 5):
                    conv_temp = convs["conv%s" %k]
                    if self.decoder_name == 'AHNet':
                        conv_temp = respath(conv_temp, self.model_depth-k, self.model_width*(2 ** 0), (3, 3))
                    ...
                    conv_temp = tf.keras.layers.Activation('sigmoid')(conv_temp)
                    conv = tf.keras.layers.concatenate([conv, conv_temp], axis=-1)
                encoder_level_5 = conv2d(conv, self.model_width * (2 ** 4), (3, 3))  # (w/16)*(h/16)*(mw*16)
            ...
            if (self.decoder_name == 'MultiResUNet') or (self.decoder_name == 'MultiResUNet3P'):
                ...
                for i in range(1, (self.model_depth + 2)):
                    ...
                    conv = conv2d(pool, self.model_width * (2 ** (i - 1)), (3, 3))
                    conv = conv2d(conv, self.model_width * (2 ** (i - 1)), (3, 3))
                    convs["conv%s" % i] = conv
                    pool = tf.keras.layers.MaxPooling2D(pool_size=(2, 2))(conv)
            else:
                for i in range(1, (self.model_depth + 2)):
        ...
        if (self.decoder_name == 'MultiResUNet') or (self.decoder_name == 'MultiResUNet3P'):
            ...
            # conv = conv2d(conv, self.model_width * (2 ** self.model_depth), (3, 3))
            # conv = conv2d(conv, self.model_width * (2 ** self.model_depth), (3, 3))

        # Feature Extractor for the AutoEncoder Mode
        if self.A_E == 1:
            # Collect Latent Features or Embeddings from AutoEncoders
            conv = extract_features(conv, self.model_width * (2 ** self.model_depth), self.feature_number)
    ...
    def ResNet50V2(self):
        ...
        if self.train_mode == 'pretrained_encoder':
            ...
            conv = conv2d(conv, self.model_width * (2 ** 0), (3, 3), activation_fun='sigmoid')
            if (self.decoder_name == 'MultiResUNet') or (self.decoder_name == 'MultiResUNet3P'):
                encoder_level_1 = multiresblock(conv, self.model_width * (2 ** 0), (3, 3), self.alpha)
                convs["conv1"] = respath(encoder_level_1, self.model_depth, self.model_width * (2 ** 0), (3, 3))  # 224*224*32
            else:
                encoder_level_1 = conv2d(conv, self.model_width * (2 ** 0), (3, 3))  # 224*224*32
                convs["conv1"] = encoder_level_1
            '''Encoder Level 2'''
            ...
            if (self.decoder_name == 'MultiResUNet') or (self.decoder_name == 'MultiResUNet3P'):
                ...
                conv = conv2d(conv, self.model_width * (2 ** 1), (3, 3), activation_fun='sigmoid')  # 112*112*64
                for k in range(1, 2):
                    conv_temp = convs["conv%s" % k]
                    if self.decoder_name == 'AHNet':
                        conv_temp = respath(conv_temp, self.model_depth - k, self.model_width * (2 ** 0), (3, 3))
                    ...
                    conv_temp = tf.keras.layers.Activation('sigmoid')(conv_temp)
                    conv = tf.keras.layers.concatenate([conv, conv_temp], axis=-1)
                encoder_level_2 = conv2d(conv, self.model_width * (2 ** 1), (3, 3))  # 112*112*64
                convs["conv2"] = encoder_level_2
            else:
                encoder_level_2 = conv2d(conv, self.model_width * (2 ** 1), (3, 3))  # 112*112*64
                convs["conv2"] = encoder_level_2
            ...
            if (self.decoder_name == 'MultiResUNet') or (self.decoder_name == 'MultiResUNet3P'):
                ...
                conv = conv2d(conv, self.model_width * (2 ** 2), (3, 3), activation_fun='sigmoid')  # 56*56*256
                for k in range(1, 3):
                    conv_temp = convs["conv%s" %k]
                    if self.decoder_name == 'AHNet':
                        conv_temp = respath(conv_temp, self.model_depth-k, self.model_width*(2 ** 0), (3, 3))
                    ...
                    conv_temp = tf.keras.layers.Activation('sigmoid')(conv_temp)
                    conv = tf.keras.layers.concatenate([conv, conv_temp], axis=-1)
                encoder_level_3 = conv2d(conv, self.model_width * (2 ** 2), (3, 3))  # 56*56*256
                convs["conv3"] = encoder_level_3
            else:
                encoder_level_3 = conv2d(conv, self.model_width * (2 ** 2), (3, 3))  # 56*56*256
                convs["conv3"] = encoder_level_3
            ...
            if (self.decoder_name == 'MultiResUNet') or (self.decoder_name == 'MultiResUNet3P'):
                ...
                conv = conv2d(conv, self.model_width * (2 ** 3), (3, 3), activation_fun='sigmoid')  # 28*28*512
                for k in range(1, 4):
                    conv_temp = convs["conv%s" %k]
                    if self.decoder_name == 'AHNet':
                        conv_temp = respath(conv_temp, self.model_depth-k, self.model_width*(2 ** 0), (3, 3))
                    ...
                    conv_temp = tf.keras.layers.Activation('sigmoid')(conv_temp)
                    conv = tf.keras.layers.concatenate([conv, conv_temp], axis=-1)
                encoder_level_4 = conv2d(conv, self.model_width * (2 ** 3), (3, 3))  # 28*28*512
                convs["conv4"] = encoder_level_4
            else:
                encoder_level_4 = conv2d(conv, self.model_width * (2 ** 3), (3, 3))  # 28*28*512
                convs["conv4"] = encoder_level_4
            ...
            if (self.decoder_name == 'MultiResUNet') or (self.decoder_name == 'MultiResUNet3P'):
                ...
                conv = conv2d(conv, self.model_width * (2 ** 4), (3, 3), activation_fun='sigmoid')  # 28*28*512
                for k in range(1, 5):
                    conv_temp = convs["conv%s" %k]
                    if self.decoder_name == 'AHNet':
                        conv_temp = respath(conv_temp, self.model_depth-k, self.model_width*(2 ** 0), (3, 3))
                    ...
                    conv_temp = tf.keras.layers.Activation('sigmoid')(conv_temp)
                    conv = tf.keras.layers.concatenate([conv, conv_temp], axis=-1)
                encoder_level_5 = conv2d(conv, self.model_width * (2 ** 4), (3, 3))  # 28*28*512
                convs["conv5"] = encoder_level_5
            else:
                encoder_level_5 = conv2d(conv, self.model_width * (2 ** 4), (3, 3))  # 14*14*1024
                convs["conv5"] = encoder_level_5
    ...
    def EfficientNetB1(self):
        ...
        if self.train_mode == 'pretrained_encoder':
            ...
            if (self.decoder_name == 'MultiResUNet') or (self.decoder_name == 'MultiResUNet3P'):
                ...
                conv = conv2d(conv, self.model_width * (2 ** 1), (3, 3))  # 112*112*64
                for k in range(1, 2):
                    conv_temp = convs["conv%s" % k]
                    if self.decoder_name == 'AHNet':
                        conv_temp = respath(conv_temp, self.model_depth - k, self.model_width * (2 ** 0), (3, 3))
            ...
            if (self.decoder_name == 'MultiResUNet') or (self.decoder_name == 'MultiResUNet3P'):
                ...
                conv = conv2d(conv, self.model_width * (2 ** 2), (3, 3))  # 56*56*256
                for k in range(1, 3):
                    conv_temp = convs["conv%s" %k]
                    if self.decoder_name == 'AHNet':
                        conv_temp = respath(conv_temp, self.model_depth-k, self.model_width*(2 ** 0), (3, 3))
            ...
            if (self.decoder_name == 'MultiResUNet') or (self.decoder_name == 'MultiResUNet3P'):
                ...
                conv = conv2d(conv, self.model_width * (2 ** 3), (3, 3))  # 28*28*512
                for k in range(1, 4):
                    conv_temp = convs["conv%s" %k]
                    if self.decoder_name == 'AHNet':
                        conv_temp = respath(conv_temp, self.model_depth-k, self.model_width*(2 ** 0), (3, 3))
            ...
            if (self.decoder_name == 'MultiResUNet') or (self.decoder_name == 'MultiResUNet3P'):
                ...
                conv = conv2d(conv, self.model_width * (2 ** 4), (3, 3))  # 28*28*512
                for k in range(1, 5):
                    conv_temp = convs["conv%s" %k]
                    if self.decoder_name == 'AHNet':
                        conv_temp = respath(conv_temp, self.model_depth-k, self.model_width*(2 ** 0), (3, 3))
