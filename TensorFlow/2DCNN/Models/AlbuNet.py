...
def trans_conv2D(x, model_width, kernel, strides):
    # 2D Transposed Convolutional Block, used instead of UpSampling
    x = tf.keras.layers.Conv2DTranspose(model_width, kernel, strides=strides, padding='same')(x)  # Stride = 2, Kernel Size = 4
    x = tf.keras.layers.BatchNormalization()(x)
    x = tf.keras.layers.Activation('relu')(x)
...
def Attention_Block(skip_connection, gating_signal, num_filters):
    # Attention Block
    conv1x1_1 = tf.keras.layers.Conv2D(num_filters, (1, 1), strides=(2, 2))(skip_connection)
    conv1x1_1 = tf.keras.layers.BatchNormalization()(conv1x1_1)
    conv1x1_2 = tf.keras.layers.Conv2D(num_filters, (1, 1), strides=(1, 1))(gating_signal)
    ...
    resampler = trans_conv2D(conv1_2, num_filters, (4, 4), (1, 1))
...
def residual_block_bottleneck(inputs, num_filters):
    ...
    # x        : input into the block
    # n_filters: number of filters
    shortcut = Conv_2D_Block(inputs, num_filters * 4, (1, 1), (1, 1))
    #
    conv = Conv_2D_Block(inputs, num_filters, (1, 1), (1, 1))
    conv = Conv_2D_Block(conv, num_filters, (3, 3), (1, 1))
    conv = Conv_2D_Block(conv, num_filters * 4, (1, 1), (1, 1))
    conv = tf.keras.layers.Add()([conv, shortcut])
    out = tf.keras.layers.Activation('relu')(conv)
...
def decoder_unit(inputs, num_filters):
    # Decoder Unit
    conv = Conv_2D_Block(inputs, num_filters, (1, 1), (1, 1))
    deconv = trans_conv2D(conv, num_filters, (4, 4), (2, 2))
    conv = Conv_2D_Block(deconv, num_filters, (1, 1), (1, 1))
...
def decoder_block(x1, x2, x3, x4, num_filters, D_S, A_G):
    ...
    levels = []
    if D_S == 1:
        level = tf.keras.layers.Conv2D(1, (1, 1), name=f'level4')(x4)
        levels.append(level)
    decode1 = decoder_unit(x4, num_filters * 8)
    shape3 = x3.shape
    x3 = Conv_2D_Block(x3, num_filters * 8, (1, 1), (shape3[1] + 1, shape3[1] + 1))
    if A_G == 1:
        x3 = Attention_Block(x3, decode1, num_filters * 8)
    decode1 = tf.keras.layers.concatenate([decode1, x3], axis=-1)
    if D_S == 1:
        level = tf.keras.layers.Conv2D(1, (1, 1), name=f'level3')(decode1)
        levels.append(level)
    decode2 = decoder_unit(decode1, num_filters * 4)
    shape2 = x2.shape
    x2 = Conv_2D_Block(x2, num_filters * 4, (1, 1), (shape2[1] + 1, shape2[1] + 1))
    if A_G == 1:
        x2 = Attention_Block(x2, decode2, num_filters * 4)
    decode2 = tf.keras.layers.concatenate([decode2, x2], axis=-1)
    if D_S == 1:
        level = tf.keras.layers.Conv2D(1, (1, 1), name=f'level2')(decode2)
        levels.append(level)
    decode3 = decoder_unit(decode2, num_filters * 2)
    shape1 = x1.shape
    x1 = Conv_2D_Block(x1, num_filters * 2, (1, 1), (shape1[1]+1, shape1[1]+1))
    if A_G == 1:
        x1 = Attention_Block(x1, decode3, num_filters * 2)
    decode3 = tf.keras.layers.concatenate([decode3, x1], axis=-1)
    if D_S == 1:
        level = tf.keras.layers.Conv2D(1, (1, 1), name=f'level1')(decode3)
        levels.append(level)
    decode4 = decoder_unit(decode3, num_filters)
    deconv5 = trans_conv2D(decode4, num_filters, (3, 3), (2, 2))
    deconv6 = Conv_2D_Block(deconv5, num_filters, (3, 3), (1, 1))
    out = Conv_2D_Block(deconv6, num_filters, (2, 2), (1, 1))
    if D_S == 1:
        level = tf.keras.layers.Conv2D(1, (1, 1), name=f'level0')(decode4)
        levels.append(level)
...
class AlbUNet:
    def __init__(self, length, width, num_channel, num_filters, ds=0, ae=0, ag=0, problem_type='Regression',
    ...
    def AlbUNet18(self):
        inputs = tf.keras.Input((self.length, self.width, self.num_channel))  # Input
...
if __name__ == '__main__':
    ...
    Model = AlbUNet(length, width, num_channel, model_width, ds=D_S, ae=A_E,  ag=A_G, problem_type='Regression', output_nums=1, pooling='avg', dropout_rate=False).AlbUNet152()
