...
def Attention_Block(skip_connection, gating_signal, num_filters, multiplier, is_transconv):
    ...
    if is_transconv:
        resampler = trans_conv2D(conv1_2, num_filters, multiplier)
...
class UNet:
    def __init__(self, length, width, model_depth, num_channel, model_width, kernel_size, problem_type='Regression',
                 output_nums=1, ds=0, ae=0, ag=0, alpha=1, feature_number=1024, is_transconv=True):
    ...
    def UNet(self):
        ...
        for j in range(0, self.model_depth):
            ...
            if self.is_transconv:
                deconv = Concat_Block(trans_conv2D(deconv, self.model_width, 2 ** (self.model_depth - j - 1)), skip_connection)
    ...
    def MultiResUNet(self):
        ...
        for j in range(1, self.model_depth):
            ...
            if self.D_S == 1:
                ...
                levels.append(level)
            if self.is_transconv:
                deconv = Concat_Block(skip_connection, trans_conv2D(deconv, self.model_width, 2 ** (self.model_depth - j - 1)))
            elif not self.is_transconv:
...
if __name__ == '__main__':
    ...
    #
    Model = UNet(length, width, model_depth, num_channel, model_width, kernel_size, problem_type=problem_type, output_nums=output_nums, ds=D_S, ae=A_E, ag=A_G, alpha=alpha, is_transconv=is_transconv).UNet()
