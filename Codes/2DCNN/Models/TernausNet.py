...
def Conv_Block(inputs, model_width, kernel, multiplier):
    ...
    x = tf.keras.layers.Conv2D(model_width * multiplier, kernel, padding='same', kernel_initializer="he_normal")(inputs)
...
def upConv_Block(inputs):
    # 2D UpSampling Block
    up = tf.keras.layers.UpSampling2D(size=2)(inputs)

    return up


def trans_conv2D(inputs, model_width, multiplier, strides):
    # 2D Transposed Convolutional Block, used instead of UpSampling
    x = tf.keras.layers.Conv2DTranspose(model_width * multiplier, (4, 4), strides=strides, padding='same')(inputs)  # Stride = 2, Kernel Size = 4
    x = tf.keras.layers.BatchNormalization()(x)
    x = tf.keras.layers.Activation('relu')(x)
...
class TernausNet:
    def __init__(self, length, width, num_channel, model_width, ds=0, ae=0, ag=0, problem_type='Regression',
    ...
    def TernausNet11(self):
        """TernausNet11 Model Design - UNet with VGG11 Encoder"""
        ...
        conv = Conv_Block(inputs, self.model_width, (3, 3), 2 ** 0)
        pool = tf.keras.layers.MaxPooling2D(pool_size=(2, 2), strides=(2, 2), padding="valid")(conv)
        convs["conv0"] = conv
        conv = Conv_Block(pool, self.model_width, (3, 3), 2 ** 1)
        pool = tf.keras.layers.MaxPooling2D(pool_size=(2, 2), strides=(2, 2), padding="valid")(conv)
        convs["conv1"] = conv

        for i in range(2, 4):
            conv = Conv_Block(pool, self.model_width, (3, 3), 2 ** i)
            conv = Conv_Block(conv, self.model_width, (3, 3), 2 ** i)
            pool = tf.keras.layers.MaxPooling2D(pool_size=(2, 2), strides=(2, 2), padding="valid")(conv)
            convs["conv%s" % i] = conv

        conv = Conv_Block(pool, self.model_width, (3, 3), 2 ** 3)
        conv = Conv_Block(conv, self.model_width, (3, 3), 2 ** 3)
        pool = tf.keras.layers.MaxPooling2D(pool_size=(2, 2), strides=(2, 2), padding="valid")(conv)
        convs["conv4"] = conv
        ...
        if self.A_E == 1:
            ...
            conv = Conv_Block(latent, self.model_width, (3, 3), 2 ** 3)
            conv = Conv_Block(conv, self.model_width, (3, 3), 2 ** 3)
        ...
        for j in range(0, 5):
            ...
            deconv = Conv_Block(deconv, self.model_width, (3, 3), 2 ** (4 - j))
            deconv = Conv_Block(deconv, self.model_width, (3, 3), 2 ** (4 - j))
            if self.is_transconv:
                deconv = Concat_Block(trans_conv2D(deconv, self.model_width, 2 ** (4 - j), 2), skip_connection)
            elif not self.is_transconv:
                deconv = Concat_Block(upConv_Block(deconv), skip_connection)
            if self.D_S == 1:
                level = tf.keras.layers.Conv2D(1, (1, 1), name=f'level{4 - j}')(deconv)
                levels.append(level)

        deconv = Conv_Block(deconv, self.model_width, (3, 3), 2 ** 0)
    ...
    def TernausNet13(self):
        """TernausNet13 Model Design - UNet with VGG13 Encoder"""
        if self.length == 0 or self.model_width == 0 or self.num_channel == 0:
            raise ValueError("Please Check the Values of the Input Parameters!")
        ...
        inputs = tf.keras.Input((self.length, self.width, self.num_channel))
        conv = Conv_Block(inputs, self.model_width, (3, 3), 2 ** 0)
        conv = Conv_Block(conv, self.model_width, (3, 3), 2 ** 0)
        pool = tf.keras.layers.MaxPooling2D(pool_size=(2, 2), strides=(2, 2), padding="valid")(conv)
        convs["conv0"] = conv
        conv = Conv_Block(pool, self.model_width, (3, 3), 2 ** 1)
        conv = Conv_Block(conv, self.model_width, (3, 3), 2 ** 1)
        pool = tf.keras.layers.MaxPooling2D(pool_size=(2, 2), strides=(2, 2), padding="valid")(conv)
        convs["conv1"] = conv
    ...
    def TernausNet16(self):
        ...
        for i in range(2, 4):
            conv = Conv_Block(pool, self.model_width, (3, 3), 2 ** i)
            conv = Conv_Block(conv, self.model_width, (3, 3), 2 ** i)
            conv = Conv_Block(conv, self.model_width, (1, 1), 2 ** i)
            pool = tf.keras.layers.MaxPooling2D(pool_size=(2, 2), strides=(2, 2), padding="valid")(conv)
            convs["conv%s" % i] = conv

        conv = Conv_Block(pool, self.model_width, (3, 3), 2 ** 3)
        conv = Conv_Block(conv, self.model_width, (3, 3), 2 ** 3)
        conv = Conv_Block(conv, self.model_width, (1, 1), 2 ** 3)
        pool = tf.keras.layers.MaxPooling2D(pool_size=(2, 2), strides=(2, 2), padding="valid")(conv)
        convs["conv4"] = conv
...
if __name__ == '__main__':
    ...
    Model = TernausNet(length, width, num_channel, model_width, problem_type='Regression', output_nums=1, ds=D_S, ae=A_E, ag=A_G, is_transconv=True).TernausNet16()
