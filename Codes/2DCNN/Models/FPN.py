...
class FPN:
    ...
    def FPN(self):
        ...
        for i in range(1, (self.model_depth + 1)):
            ...
            lateral = tf.keras.layers.Conv2D(1, (1, 1))(conv)
        ...
        for j in range(0, self.model_depth):
            ...
            if self.is_transconv:
                deconv = Add_Block(skip_connection, trans_conv2D(deconv, self.model_width, 2 ** (self.model_depth - j - 1)))
        ...
        if self.problem_type == 'Classification':
            outputs = tf.keras.layers.Conv2D(self.output_nums, (1, 1), activation='softmax', name="out")(deconv_tot)
        elif self.problem_type == 'Regression':
            outputs = tf.keras.layers.Conv2D(self.output_nums, (1, 1), activation='linear', name="out")(deconv_tot)
...
if __name__ == '__main__':
    ...
    model_name = 'FPN'  # FPN
    #
    Model = FPN(length, width, model_depth, num_channel, model_width, kernel_size, problem_type=problem_type, output_nums=output_nums, ds=D_S, ae=A_E, ag=A_G, is_transconv=True).FPN()
