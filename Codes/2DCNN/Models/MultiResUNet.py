...
def MultiResBlock(inputs, model_width, kernel, multiplier, alpha):
    ...
    shortcut = inputs
    shortcut = Conv_Block(shortcut, int(w * 0.167) + int(w * 0.333) + int(w * 0.5), (1, 1), multiplier)

    conv3x3 = Conv_Block(inputs, int(w * 0.167), (kernel, kernel), multiplier)
    conv5x5 = Conv_Block(conv3x3, int(w * 0.333), (kernel, kernel), multiplier)
    conv7x7 = Conv_Block(conv5x5, int(w * 0.5), (kernel, kernel), multiplier)
...
def ResPath(inputs, model_depth, model_width, kernel, multiplier):
    ...
    shortcut = inputs
    shortcut = Conv_Block(shortcut, model_width, (1, 1), multiplier)

    out = Conv_Block(inputs, model_width, (kernel, kernel), multiplier)
    out = tf.keras.layers.Add()([shortcut, out])
    out = tf.keras.layers.Activation('relu')(out)
    out = tf.keras.layers.BatchNormalization()(out)
    ...
    for _ in range(1, model_depth):
        ...
        out = Conv_Block(out, model_width, (kernel, kernel), multiplier)
        out = tf.keras.layers.Add()([shortcut, out])
        out = tf.keras.layers.Activation('relu')(out)
        out = tf.keras.layers.BatchNormalization()(out)
...
class UNet:
    def __init__(self, length, width, model_depth, num_channel, model_width, kernel_size, problem_type='Regression',
                 output_nums=1, ds=0, ae=0, ag=0, lstm=0, alpha=1, feature_number=1024, is_transconv=True):
        ...
        for i in range(1, (self.model_depth + 1)):
            for j in range(0, (self.model_depth - i + 1)):
                if (i == 1) and (j == (self.model_depth - 1)):
                    ...
                    if self.is_transconv:
                        deconv = Concat_Block(skip_connection, trans_conv2D(convs_list[j + 1], self.model_width, 2 ** j))
                    elif not self.is_transconv:
                    ...
                    if self.is_transconv:
                        deconv = Concat_Block(skip_connection, trans_conv2D(deconvs["deconv%s%s" % ((j + 1), (i - 1))], self.model_width, 2 ** j))
                    elif not self.is_transconv:
            ...
            for j in range(0, (self.model_depth - i + 1)):
                if (i == 1) and (j == (self.model_depth - 1)):
                    ...
                    if self.A_G == 1:
                        skip_connection = Attention_Block(convs_list[j], deconvs["deconv%s%s" % ((j + 1), (i - 1))], self.model_width, 2 ** j)
                    if self.is_transconv:
                        deconv = Concat_Block(skip_connection, deconv_tot, trans_conv2D(deconvs["deconv%s%s" % ((j + 1), (i - 1))], self.model_width, 2 ** j))
                    elif not self.is_transconv:
    ...
    def MultiResUNet3P(self):
        ...
        for j in range(0, self.model_depth):
            ...
            for k in range(0, self.model_depth - j - 1):
                ...
                skip_connections_all = tf.keras.layers.concatenate([skip_connections_all, skip_connection], axis=-1)
            deconv_tot = upConv_Block(deconv, size=(2 ** 1,2 ** 1))
            deconv_tot = MultiResBlock(deconv_tot, self.model_width, self.kernel_size, 2 ** 0, self.alpha)
            deconv_tot = tf.keras.layers.concatenate([skip_connections_all, deconv_tot], axis=-1)
            if j > 0:
                for m in range(0, j):
                    deconv = upConv_Block(deconvs["deconv%s" % m], size=(2 ** (j-m),2 ** (j-m)))
                    deconv = MultiResBlock(deconv, self.model_width, self.kernel_size, 2 ** 0, self.alpha)
                    deconv_tot = tf.keras.layers.concatenate([deconv_tot, deconv], axis=-1)
            deconv = MultiResBlock(deconv_tot, self.model_width, self.kernel_size, self.model_depth, self.alpha)
            deconvs["deconv%s" % j] = deconv
            if self.D_S == 1:
                # For Deep Supervision
                level = tf.keras.layers.Conv2D(1, (1, 1), (2, 2), name=f'level{self.model_depth - j}')(deconv)
...
if __name__ == '__main__':
    # Configurations
    length = 224  # Length of each Segment
    width = 224  # Length of each Segment
    model_name = 'UNet3P'  # UNet or UNetPP
    ...
    Model = UNet(length, width, model_depth, num_channel, model_width, kernel_size, problem_type=problem_type, output_nums=output_nums,
                 ds=D_S, ae=A_E, ag=A_G, lstm=LSTM, alpha=alpha, is_transconv=is_transconv).UNet3P()
